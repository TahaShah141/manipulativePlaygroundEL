...
const OnesBlock = forwardRef<HTMLDivElement, BlockProps>(({attributes, listeners, style, isDragging, isSmall=false}, ref) => {
  return (
    <div ref={ref} {...attributes} {...listeners} style={style} className={`flex justify-center items-center ${isSmall ? "size-3 lg:size-4" : "size-4 lg:size-5 xl:size-6"} transition-colors duration-300 border-2 border-black rounded-sm lg:rounded-md ${isDragging ? "bg-neutral-200" : "bg-brown"}`} />
  )
})
...
const TensBlock = forwardRef<HTMLDivElement, BlockProps>(({attributes, listeners, style, isDragging, isSmall=false}, ref) => {
  return (
    <div ref={ref} {...attributes} {...listeners} style={style} className={`flex flex-col transition-colors duration-300 rounded-sm lg:rounded-md xl:rounded-lg border border-black overflow-hidden ${isDragging ? "bg-blue-200" : "bg-blue-500"}`}>
      {Array.from({length: 10}, (_, i) => (
        <div key={i} className={`${isSmall ? "size-3 lg:size-4" : "size-4 lg:size-5 xl:size-6"} border-black border`} />
      ))}
    </div>
  )
})
...
const HundredsBlock = forwardRef<HTMLDivElement, BlockProps>(({attributes, listeners, style, isDragging, isSmall=false}, ref) => {
  return (
    <div ref={ref} {...attributes} {...listeners} style={style} className={`grid grid-cols-10 grid-rows-10 transition-colors duration-300 rounded-sm lg:rounded-md xl:rounded-lg border-black border-2 overflow-hidden ${isDragging ? "bg-yellow-100" : "bg-yellow-300"}`}>
